...
- **Widgets**: `lib/widgets/` — `vida_verse_card.dart` (verse + share/save actions) and `tool_card.dart` (icon + title + subtitle card).
...
- **Native provider**: `FavoritoWidgetProvider` (`android/.../com/vida/project/`). Same pattern as ContraPecado — `setImageViewResource` + `clipToOutline`. If verse is >80 chars, text size drops from 13sp to 12sp.
